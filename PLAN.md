...
- 真正原因：同一 process_code 下混两套字段结构，解析映射被迫做新旧分支判断，容易静默错位（组件 ID 复用、字段改名）；且新数据无里程后，里程偏差/油费等统计口径在同一表单内断裂，无法向报告读者解释分界点。
- 新表单 = 新 process_code，`raw_approvals.process_code` 天然成为 schema 版本号，出问题可切回旧表单回滚。

#### 总体原则：存量冻结、增量走新口径

- 旧 process_code 的解析映射保持不变，历史 visits / 里程 / 里程偏差异常原样保留，旧映射代码保留用于历史数据重跑。
- 新 process_code 起一套新映射；新数据 `reported_distance_km` 为空。
- 整体分析逻辑（停留检测、轨迹、拜访类异常、风险评分、报告）保持不变，仅里程相关部分对新单自动失效/降级。

#### 实施前置（业务方）

- [ ] 在钉钉后台新建审批表单（删除填报里程、新增拜访情况字段），取得新 process_code。
- [ ] 新表单上线后，将旧表单从工作台下架/停用，避免双轨提交（否则旧单持续产生但已切换 env 后无法同步）。

#### 等新表单字段确定后实施

- [ ] 新增新 process_code 的字段映射解析；拜访情况新字段进 `visits` 的列设计一次定清（加列还是扩展 JSON），报告（日/周/月报）是否使用这些字段同步确定。
- [ ] 更新后端 `DINGTALK_PROCESS_CODE` 环境变量（当前仅支持单个 process_code，见 `backend/src/services/dingtalk.ts:28`）。

#### 现在即可先做（与新表单字段无关，暂未启动）

- [ ] 里程类异常规则（`mileage_deviation`、`mileage_reading_invalid`、油费估算相关）对 `reported_distance_km IS NULL` 的审批单**跳过而非误报**——新旧平滑切换的关键，对存量零影响。
- [ ] 确认解析分发以 `raw_approvals.process_code` 作为版本依据，为切换过渡期"两个 code 并存"（旧表单在途审批单）留口子。
