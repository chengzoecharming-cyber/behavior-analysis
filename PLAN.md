...
> 最后更新：2026-07-30
> 当前重点：住址白名单修复已上线，**洪晨备住址未匹配待后续人工确认**。
...
### P0-4 周期风险-里程偏差按审批单聚合（已修复）

#### 问题

- 里程偏差异常原先按“相邻签到点分段”生成，导致一个审批单在周期风险 timeline 中出现多条记录。
- 当某段估算距离极小（如 <1 km）时，偏差率会异常放大，出现 `60566.7%` 这类极端数值。
- 周期风险 timeline 中同一日期存在多个审批单时，出现多个并列 dot，不符合时间线阅读习惯。

#### 修复

- **后端 `backend/src/services/anomalyDetection.ts`**：
  - 按 `approval_id` 聚合同一审批单的全部里程段，计算总填报里程 / 总估算里程 / 总偏差率。
  - 仅在审批单起始业务日生成一次异常，避免跨天审批单重复生成。
- **后端 `backend/src/services/mileageAnalysis.ts`**：
  - 无 `approval_id` 的数据按 `user_id + 业务日期` 兜底分组，避免把不同日期的 Excel 导入记录错误聚合。
- **前端 `frontend/src/pages/ConsolePage.tsx`**：
  - 周期风险-里程偏差不再展示分段明细。
  - 按日期分组 timeline，同一日期下有多个审批单时只显示一个 dot，日期下列出所有审批单。

#### 验收

- 前后端 `npm run build` 通过。
- 本地重新运行 `npm run recompute:anomalies`，794 个 user/date 对无报错。
- 抽查「邹闽」：6.26 仅保留一条 `审批单 ...00485407 填报 297 km / 估算 81.22 km / 超出 265.7%`，原 `60566.7%` 异常消失。
- 相关提交：`f59036a`、`cb1e95a`。
...
- [x] **F9. 行为报告发送到钉钉文档（知识库）**
  - 全公司 / 部门 / 子部门 / 个人周期报告均支持生成并写入钉钉文档
  - 文档目录结构：`外勤行为分析报告 / 公司报告 / {日报,周报,月报}`、`01. 销售部 / 00. 部门报告 / {日报,周报,月报}`、`01. 销售部 / 01. 子部门 / 员工姓名 / {日报,周报,月报}`
  - 目录支持手动 `XX. ` 序号前缀，代码按 base 名称匹配
  - 自动同步调度：日报每天 21:00、周报每周日 18:00、月报每月 30 日 18:00（北京时间）
  - 范围：最近 14 天有数据的节点/人员；无数据时生成「无拜访」占位报告
  - 实现：`backend/src/services/reportGenerationService.ts`、`dingtalkDoc.ts`；调度入口 `startReportGenerationScheduler()`
  - 前端控制台顶部筛选区增加「导出到钉钉文档」按钮
  - 后端手动重跑接口：`POST /export/generate-reports`（`daily|weekly|monthly`）
  - 待完善：AI 摘要、地图截图嵌入、消息通知
...
- [x] **F10.5 控制台级联选择器支持父部门直属人员 & 钉钉同步按最深部门归属**
  - 问题：从部门排行榜点击李朝晖跳转到个人控制台后，级联选择器空白，仅右侧显示「当前：个人 李朝晖（date）」。
  - 根因：
    1. 前端 `ConsolePage` 渲染级联选择器时，父部门有子部门的情况下会丢弃父部门直属人员；
    2. 后端 `syncContacts` 按部门遍历顺序决定 `source_dept_id`，李朝晖被先遍历到的「销售部」收走，未落到其真实子部门。
  - 修复：
    1. 前端 `buildCascaderData` 为父部门直属人员增加「直属」虚拟子节点；`getCascaderValueFromState` 同步支持从父部门 `userIds` 中定位用户；`parseCascaderValue` 将「直属」节点按部门范围解析。
    2. 后端 `syncContacts` 先聚合每个用户的完整 `dept_id_list`，再按部门深度取最深部门作为 `source_dept_id`；仅挂在父部门的人员（如总 leader）仍保留在父部门。
  - 文件：`frontend/src/pages/ConsolePage.tsx`、`backend/src/services/dingtalk.ts`
  - 本地验证（2026-07-16）：前后端 `npm run build` 通过。
...
- [x] **住址白名单（home 地址）**：已支持从 `backend/data/employee_addresses.xlsx` 导入员工常住地址到 `users.home_address`，重复签到检测时自动排除命中住址的 visit。LCS 子串匹配 + 地理编码 500 米半径兜底，可处理门牌号/楼栋号插入等模糊地址。
- [ ] **办公室白名单**：管理员维护办公室地址/坐标，重复签到检测时排除。待业务方提供办公室地址表。
- [ ] **洪晨备住址未匹配**：2026-07-20 导入员工住址时，Excel 中的「洪晨备」在系统 `users` 表和 `visits` 表均无匹配记录，暂无法自动写入 `home_address`。后续需人工确认其在系统中的真实姓名，或补充钉钉通讯录同步后重新导入。
- [ ] **周期总览导出并发钉钉**：个人/组织报告均支持发送到当前群聊。
- [ ] **日历视图页面优化**：调整单日视图布局。
- [ ] **地图加载优化**：排查 40 秒加载慢的原因（地图脚本/数据量/路径规划），针对性优化。
- [ ] **网站栏目标题优化**：统一导航命名，提升可读性。
- [ ] **手动调整 `dingtalk_users.source_dept_id`**：管理员页面允许修正特殊角色（如 sub leader、跨部门人员）在组织架构树中的默认归属部门。优先级低，待有明确业务场景后再实现。
...
- [x] 已执行：本地和线上分别运行 `npm run recompute:anomalies`（2026-07-20）
...
## 异常标签体系三层分类（2026-07-16 讨论确定）

> 结论：异常标签不再统一作为「违规判定」，而是按用途分为三层，分别服务于数据修正、行为观察与系统判定。

### 分层定义

| 层级 | 定义 | 用途 | 前端展示位置 |
|---|---|---|---|
| **事实层** | 填报或操作本身存在的问题 | 指出数据缺漏，可推给员工补全 | 轨迹内容 Timeline、导出报告 |
| **分析层** | 对员工行为模式的描述 | 供管理者观察判断，不下结论 | Console Page 卡片、导出报告 |
| **判定层** | 基于数据可直接得出的统计结论 | 作为管理参考，但不直接处罚 | Console Page 卡片（暂缓外显） |

### 规则清单与状态

| 层级 | 规则 key | 名称 | 状态 | 说明 |
|---|---|---|---|---|
| 事实层 | `mileage_reading_invalid` | 填报异常 | active | 出发/终点读数缺失、读数倒挂、超上限 |
| 事实层 | `missing_special_reason` | 特殊签到缺原因 | active | 特殊签到未填写原因 |
| 分析层 | `duplicate_location` | 重复签到 | active | 过去两周同一地点重复签到 ≥ 8 次 |
| 判定层 | `low_visit_count` | 拜访量不足 | active | 过去 5 个工作日累计签到 < 15 次 |
| 判定层 | `mileage_deviation` | 里程偏差 | active | 填报里程 vs 高德推荐差异较大 |
| 判定层 | `route_detour` | 路径绕行 | 已弃用 | 两点直线距离口径已弃用 |
| 判定层/事实层 | `invalid_trip_type` | 异常出行方式 | deactivate | 出行方式与里程不匹配 |
| 分析层 | `long_idle` | 长时间未移动 | deactivate | 相邻签到间隔较长 |
| 分析层 | `long_stop` | 停留过长 | deactivate | 同地点多次打卡聚合，口径不清 |

### 关键决策

1. **重复签到阈值**
   - 过去两周同一地点 ≥ 8 次
   - 后续需补充住址/办公室白名单，避免出发点等固定位置误报

2. **停留时长/停留过久规则暂缓**
   - 原因：钉钉数据多为单次打卡，无法精确计算停留时长；出差住宿、吃饭、客户难缠等都会导致停留久，解释理由难证伪
   - 待补充：住址/办公室白名单、拜访内容 300 字强制填写、订单/出货数据接入后再评估

3. **判定层规则保留 active，但降低展示强度**
   - 拜访量不足、里程偏差作为「数据结论」展示，不直接等同于「异常」
   - 文案避免使用「异常」「违规」等定性词汇
...
5. **不同层级展示位置**
   - Console Page 卡片：分析标签 + 判定标签（以数据结论形式展示）
   - 轨迹内容 Timeline：事实标签
   - 导出报告：事实标签 + 分析标签

---

## 风险内容更新（2026-07-20 讨论确定）

> 来源：业务方对风险规则、周期口径、展示入口的重新梳理。
> 核心原则：**以 ConsolePage 作为主要风险展示入口，按业务周/月查看；判定层风险简化为 3 条，其余禁用。**

### 业务周期定义

- 以每年 **6 月 1 日（周一）** 为第 1 周起点。
- 业务周与自然周重合：**周一 00:00 ~ 周日 23:59**。
- 月：当前先按自然月，后续再决定是否改为业务月。

### 启用的风险规则（判定层，参与评分）

| 规则 key | 名称 | 触发条件 | 周期 | 权重 |
|---|---|---|---|---|
| `low_visit_count` | 拜访量不足 | 当前完整业务周拜访量 < 10 次 | 当前业务周（仅在周日展示） | 0.25 |
| `duplicate_location` | 重复签到 | 当前业务周（周一到当前日）同一地点签到 ≥ 3 次 | 当前业务周已过的部分 | 0.15 |
| `mileage_deviation` | 里程偏差 | 填报里程 > 高德估算里程 × 1.30 | 单日（按审批单分段） | 0.20 |

### 禁用规则

以下规则暂不启用（`enabled = false`）：
- `long_stop` 停留过长
- `long_idle` 长时间未移动
- `route_detour` 路径绕行
...
- 员工住址（已有 `users.home_address` + 白名单匹配）
- 公司地址：`广东省深圳市宝安区创维数字大厦`（新增全局排除）

### ConsolePage 展示方案

#### 1. 日期选择器快捷选项

在 DatePicker 左侧垂直排列 5 个快捷选项：
- 本周
- 上周
- 过去两周
- 过去三周
...
点击后自动设置对应 date range，不支持今日/昨日。

#### 2. 单日视图

- 里程偏差：每日展示
- 重复签到：按当前业务周（周一到当前日）实时计算，每日展示
- 拜访量不足：仅在当前业务周周日展示本周数据；非周日不展示

#### 3. 范围视图（OverviewPanel）

- 选择 5 个快捷选项之一时，每日趋势图上方的 alert 区域展示三个风险 tag。
- 每个 tag 可展开，展开后展示该 tag 对应的具体日期（与现有 UI 一致，只改内容）。
- 月范围（上月）与周范围展示方式相同。

#### 4. 组织视角风险展示（待实现）

ConsolePage 作为统一查询入口，已支持全公司 / 部门 / 子部门 / 个人四级查询。当前组织视角（`scope != person`）只展示排行榜和热力图，缺少风险聚合。需要补充：

- **统一的风险列**：在 `OrgQueryPanel` 排行榜中增加风险列，展示 4 个 tag：
  - 拜访量不足（判定层，参与评分）
  - 重复签到（判定层，参与评分）
  - 里程偏差（判定层，参与评分）
  - 填报异常（事实层，不参与评分，仅展示）
- **展示规则**：在公司 / 部门 / 子部门任意维度下，排行榜结构保持一致，都有风险列。只要该节点下级有命中某规则，就在对应行显示该 tag；不展示具体数量、比例或异常详情。
- **下钻规则**：点击部门 / 子部门 / 人员行，新标签页打开 ConsolePage 并带入当前 filter（`scope`、`node`、`user`、`start`、`end` 保持一致）。进入下一层后，才能看到该层更详细的风险内容（如个人维度的具体异常描述）。
- **风险指标卡**：不展示“命中多少人”这类负向指标。改为展示正向指标，如：
  - 拜访达标率：周期内拜访量达标人数 / 总人数
  - 里程准确率：周期内无里程偏差审批单数 / 有里程审批单总数
  - 其他正向指标待补充
- **Tag 样式**：沿用 ConsolePage 当前颜色，本期不重新设计。
- **后端接口**：复用 `/analytics/org-overview` 增加 `risk` 相关字段，或新增 `/analytics/org-risk-overview`，基于 `anomalies` 表和当前启用规则聚合，不依赖 `risk_summary_cache`。
- **实现顺序**：先完成 ConsolePage 高维度风险聚合，再改造 DecisionPage。

#### 5. 组织视角布局与省份分布（待实现）

- **布局调整**：非人员视角下，将「趋势分析」与「拜访热力图」放在同一行（等宽），下方放置「排行榜」及「其他卡片」。
- **省份分布 Donut Chart**：在组织视角增加一个 donut chart，展示拜访次数按省份分布。默认展示前 5 个省份，其余合并为「其他」。公司视角意义最大，部门 / 子部门视角可酌情展示或折叠。
- **数据来源**：当前 `visits` 表没有 `province` 字段，需从 `address` 中按前缀提取省份，或后续通过 geocoding 补全。具体方案待实现时确定。
- **其他卡片**：人均拜访次数、客户覆盖数。

### 文案规范

异常描述中不展示「第 N 周」，直接展示日期范围，例如：
- 拜访量不足：`2026-06-02 ~ 2026-06-08 拜访量 8 次，低于 10 次阈值`
- 重复签到：`2026-06-09 ~ 2026-06-11 同一地点重复签到 3 次，超过 2 次阈值`
- 里程偏差：`A → B 填报 15 km vs 估算 10 km，超出 50%`

### 待后续处理

- **ConsolePage 与 DecisionPage 定位确认**：
  - ConsolePage 是统一查询入口，承载全公司 / 部门 / 子部门 / 个人四个层级的查询与分析。
  - DecisionPage 是 Dashboard（全局风险驾驶舱），负责全局风险总览与下钻入口，不重复做查询页功能。
- **下一步顺序**：先完成 ConsolePage 的「组织视角风险展示」（高维度风险聚合），待稳定后再改造 DecisionPage。
- **DecisionPage 改造方向（先写入计划）**：
  - 去掉 `高风险 / 可疑 / 正常` 分级卡片，因为风险评分算法仍处于暂缓状态，分级缺乏可靠依据。
  - 顶部展示正向指标卡：拜访达标率、里程准确率等（与 ConsolePage 组织视角保持一致）。
  - 部门风险排行表：按部门展示风险 tag（规则同 ConsolePage 组织视角），点击下钻 ConsolePage 部门 / 子部门视图。
  - 区域拜访热力图保留，但折叠或置于第二屏，作为补充分析。
  - 数据接口：复用 ConsolePage 高维度风险聚合接口，不单独新增。
- **总览入口与日期定位**：当前由 ConsolePage 的快捷选项承担周期总览；全局视角（公司/部门维度）后续在 DecisionPage 实现。
...
### Step 3.6：钉钉审批表单版本切换（新建表单 + 新映射）

> 记录时间：2026-07-30；方案确认：2026-08-05；上线：2026-08-07
> 状态：**已上线**。旧表单已停用（2026-08-07），新表单启用；**2026-08-06 18:36 之前的 v2 单均为测试数据，不入库/需清除**

#### 背景与决策

钉钉原审批表单计划修改字段：**删除「填报里程」，新增拜访情况相关字段**。已确认方案为**新建表单（新 process_code）**，不在原表单上修改。

- 旧表单（v1）：「用车里程登记&客户签到」，`PROC-F16F0514-AEE4-4E74-BBE3-574D19687EFC`
- 新表单（v2）：「用车里程登记&拜访客户签到」，`PROC-1BF061D3-38F0-4961-B41D-D41CDDDF9212`

决策理由：

- 历史数据无论如何都安全：钉钉侧历史审批单是提交时快照，改模板不回溯；系统侧原始数据完整保留在 `raw_approvals`。所以"怕影响旧数据"不是真正原因。
- 真正原因：同一 process_code 下混两套字段结构，解析映射被迫做新旧分支判断，容易静默错位（组件 ID 复用、字段改名）；且统计口径在同一表单内断裂，无法向报告读者解释分界点。
- 新表单 = 新 process_code，`raw_approvals.process_code` 天然成为 schema 版本号；**解析分发一律按 process_code，不按表单中文名**，出问题可切回旧表单回滚。

#### v2 表单结构（2026-08-05 从钉钉 API 拉取确认；2026-08-06 改版已同步）

- `出行方式`（DDSelectField）：`开车（统计里程+签到）` / `其他（签到）`，带行为联动——选「其他」时车辆、出发前读数、起终点打卡、终点读数、今日出行总里程全部隐藏（测试单证实均为 null）
- `选择出行车辆`（CascadeField）、`出发前里程读数`（NumberField）
- 拜访块 ×5：`TimeAndLocationField` 打卡 → `拜访客户N`（N=1 为 OpenDataField；N=2~5 嵌在 TableField 内）→ `现场交流照片N` → `沟通内容详情N` / `存在问题点N` → `是否前往下一地点N+1`
  - 2026-08-06 改版：删除「交流对象」字段；两个内容字段名加编号（`沟通内容详情1`…）
- 结尾：终点打卡（TimeAndLocationField）→ `终点里程读数` → `今日出行总里程`（CalculateField，= 终点−出发前，两值同源必然一致）
- **`AI总结`（DDAIField，2026-08-06 新增）**：整单一个字段，钉钉侧 AI 按块生成「总结内容1/总结内容2/…」，N 与拜访块序号对齐
- v1 的特殊签到（打卡地/特殊签到原因/签到图片）、逐段终点读数、逐段累计里程、签到次数、虚拟客户出行方式选项**全部删除**
- **拜访客户N 为必填**：v2 拜访块必有客户名，「客户名为空」只出现在开车单的起点/终点打卡（无客户字段，落入情况B 只作里程依据）及个别异常快照

#### 核心口径：「出行方式」×「客户名称」，无起终点概念

内部判断不区分起点/终点（起终点只是开车单前端呈现的字段）。每个打卡点按客户名称逐个判定（拆分多客户名后）：

- **情况A**：存在非占位的真实客户名（住址/公司/酒店不拦截，照计拜访）
- **情况B**：真实客户名数为 0——客户名为空，或全部为占位名（占位 = 匹配 `/虚拟|签到用/`，覆盖「虚拟客户」家族与 CRM 遗留占位「住址/公司（签到用）」；混合填写时真实客户照计）

| | 情况A | 情况B |
|---|---|---|
| 开车 | 计里程 + 计拜访 | 计里程，不计拜访 |
| 其他 | 展示轨迹 + 计拜访 | 展示轨迹，不计拜访 |

推论（已与业务方确认）：

- **有真实客户名即算拜访，即使在住址/公司/酒店**——v2 数据不再应用地址白名单排除；`exclude_from_visit_count` 对 v2 = 情况B 判定。v1 及 Excel 数据的地址排除逻辑完全不变。
- **一个打卡点填 N 家客户 = N 次拜访**：`visits.customer_count` 记录该打卡点客户数（v2 按 TableField 行数/OpenDataField 单值计算，无客户为 0；v1/Excel 按 customer_name 分隔符 `[,，、]` 拆分计，空名单值按 1——v1 是地址制口径，无客户名的有效签到点仍计 1 次）。全系统「拜访次数」类统计从 `COUNT(*)` 改为 `COALESCE(SUM(customer_count), 0)`（决策页/控制台/区域热力/风险摘要/日周月报/导出报告共 9 个文件约 15 处）；轨迹、停留点、里程、热力图点位数不受影响的仍按行数。
- **「客户数」同步拆分**：`COUNT(DISTINCT customer_name)` 会把「A、B」整体算一个客户，已改为 JS 侧 `splitCustomerNames` 拆分去重（orgService totalCustomers、companyDashboard 汇总与部门雷达 customer_coverage、reportGenerationService 每日/全局 customer_count、报告「拜访客户数」与「客户拜访列表」——列表中多客户拆成多行、每家各计一次，涉及人员按拆分后匹配）。SQL 侧不展开 LATERAL（会导致同行其他聚合翻倍），一律单独查询 + JS 拆分。存量 v1 数据用 `npm run backfill:customer-count` 回填（12 行实测，幂等）。
- 已知口径不一致（暂未动）：报告全局 customer_count（reportGenerationService 原 globalCustomerResult 路径）不加 exclude_from_visit_count 过滤，与每日口径不一致，属历史行为，需要时单独决策。
- 开车单的出发点/终点打卡天然无客户名 → 情况B，只作里程依据，无需位置特判。
- 里程口径：**总里程（填报）= 终点里程读数 − 出发前里程读数**（挂在最后一个打卡点）；中间拜访点无逐段读数；**估算里程 = routes 按实际轨迹点分段叠加**（原有逻辑，不受影响）。
- 「其他」单无车辆/里程概念，一个打卡点 = 一个签到，轨迹照画、routes 照算。

#### v2 实例值格式（2026-08-05 测试单证实）

- 手动输入客户：纯文本（如 `腾讯`）
- CRM 下拉客户：`客户名称:龙兴（杭州）…` 字符串（剥前缀）；TableField 内取 `rowValue[i]` 中 OpenDataField 项的 `value`，`extendValue` 仅作 fallback；CRM 关联带 `relateId`（`crm_customer`），可与手动输入区分（暂不存储该区分）
- **未填写字段不是 null，是 schema 回显 JSON**（组件定义，含 `children`/`props`，无 `rowValue`/真实 `value`）——解析必须显式判空
- 详情接口不返回 `originator_user_name`，姓名走现有通讯录 fallback

#### 字段映射（v2 → ParsedVisit，输出字段与 v1 完全一致）

| v2 表单字段 | 映射 |
|---|---|
| 出行方式 | `trip_type`（原文），是否开车决定里程字段解析 |
| 选择出行车辆 | `vehicle`（复用 parseVehicle） |
| 出发前里程读数 | 第一个打卡点 `start_odometer` |
| 终点里程读数 | 最后一个打卡点 `end_odometer`；`reported_distance_km = 终点 − 出发前`（合法性校验沿用 MAX_MILEAGE_KM） |
| 今日出行总里程 | 最后一个打卡点 `cumulative_mileage_km` |
| TimeAndLocationField | 打卡点（time/lat/lng/address），`location_name` = 出发点/签到点N |
| 拜访客户N（打卡点之后，向后找） | `customer_name` |
| 现场交流照片N | `photos` |
| AI总结（DDAIField，按「总结内容N」拆块） | `visit_note`（首选）；无值时兜底拼接 `沟通内容详情N`/`存在问题点N`（旧快照还有「交流对象」），保留原文 |

#### 任务拆分

- [x] 配置：新增 `DINGTALK_PROCESS_CODE_V2` 环境变量（`.env.example`、本地 `.env` 已加；**服务器 `.env` 待部署时添加**）
- [x] 同步侧：`getApprovalInstances`/`fetchAllApprovalIds`/`syncApprovals` 支持多 process_code 轮询；解析分发按 process_code（`saveRawApproval` 落库 process_code）
- [x] v2 解析器 `parseApprovalInstanceV2`：schema 回显判空、TableField 客户提取、拜访情况三字段拼接、里程字段按字段名直取
- [x] `normalization.ts`：`ParsedVisit.form_version='v2'` 时 `exclude_from_visit_count` 按情况B 判定，跳过地址白名单；`backfillVisitExclusion` 脚本同步分流
- [x] `lineageViewerService` 重新解析路径按 `raw_approvals.process_code` 分发
- [x] 用测试单 202608051339000482892（开车）与 202608051148000213179（其他）验证解析输出（2026-08-05 通过）
- [x] 更新 AGENTS.md（双表单、v2 口径）
- [ ] 周五上线：按下方「上线步骤」执行（部署、开开关、清理测试单、观察首日）
- [ ] （后续，等业务方导出新格式 Excel）`excelParser.ts` 宽表新增 v2 映射
...
- [x] **验证 AI总结**（2026-08-06 通过，测试单 202608061542000384525 / 202608061539000381706）：DDAIField 值格式为 `总结内容1:…`（半角冒号、每块一行），**块编号 = 拜访块编号（拜访客户N），与打卡点序号无关**，起点/终点不占编号；解析器按「有客户块的打卡点顺序」对齐，已验证对应正确。

#### 2026-08-06 测试单实测补充结论

- **TableField 拜访客户可填多行**：一个打卡点可填多家客户（如「咖啡行」「微微公司」两行 rowValue），解析器收集全部行去重后以「、」连接为 `customer_name`，并写 `customer_count`（该点客户数）；拜访次数统计按 `SUM(customer_count)` 计。
- **RUNNING 单防误标**：审批单未完成时终点打卡/读数未填，最后一个已填打卡点不再误标「终点」、不追加「缺少终点里程读数」；「终点」标签只在终点读数有值时出现，缺读数备注只在审批 COMPLETED 后追加——COMPLETED 的开车单最后一个打卡点即终点打卡（表单必填），与原设计一致。
- AI总结为空的块（或整字段为空）自动回退到 `沟通内容详情N`/`存在问题点N` 拼接，新旧快照均兼容。

#### 已知边界：占位客户名与住址排除（2026-08-06 莫龙案例诊断）

案例：莫龙 8/2 签到点3 的 customer_name 为「住址（签到用）」，未被 `exclude_from_visit_count` 排除，混入拜访次数。诊断（只读排查，未修改）：

- 该签到坐标 (34.84786, 113.52992) 与其登记住址坐标 (34.846901, 113.538595) 相距约 **800 米，超出 500 米匹配半径**；
- 文本匹配也失效：签到地址是高德反向解析的「中原区西四环323号」，登记住址是「高新区郭村新城A区」，行政区划命名不一致，无子串匹配；
- v1 排除口径只看地址，不读 customer_name 的自描述（「住址（签到用）」）。

可选项（未决策）：人工修正该员工 home 坐标 / 放宽匹配半径 / 利用 customer_name 自描述。

**对 v2 口径的影响（2026-08-06 已决策）**：v1 历史数据（含客户列表误滤「虚拟客户（签到用）」、住址混入等）认定为源头数据问题，**不修**——v1 客户字段只能下拉选择，名字是 CRM 选项本身带的；v1 拜访次数维持纯地址制口径（不看客户名，虚拟客户打卡一直计入拜访，与「v1 虚拟客户=合作伙伴/陌拜/展会真实拜访」的新认知恰好一致）。

v2 按「打卡 + 客户名称」判定，与地址无关：**占位写法 = 虚拟客户家族 + 「（签到用）」类 CRM 遗留占位**（业务约定：住址、公司、酒店类未真实拜访的打卡，源头统一写「虚拟客户」；v2 支持手动输入，源头名字干净）。逐个客户名判定：非占位名数为 0 → 情况B 不计拜访；混合填写（真实客户 + 占位）真实客户照计；多客户按个数计拜访。实现：`splitRealCustomerNames`（normalization.ts），模式 `/虚拟|签到用/`。
- [ ] **观察首日**：查看 `dingtalk_sync_logs`（missing_count/duplicate_count 应为 0）、同步健康告警、数据血缘页抽查几张 v2 单的三步对照（原始表单 → 重新解析 → 已入库）。
- [ ] **回滚预案**：清空 `DINGTALK_PROCESS_CODE_V2` 重启即退回只同步旧表单；已入库的 v2 数据不受影响（可按 approval_id 定向删除后重跑）。

#### 混用期口径（v1/v2 并存时无需额外开发）

- 控制台/地图按 user + business_date 查 visits 天然合并；轨迹按 approval_id 分组回放。
- 日/周/月报基于 visits 聚合，自动合并；拜访次数按各行写入时已定的 `exclude_from_visit_count` 过滤。
- 里程：v1 逐段读数与 v2 全程读数同落 `reported_distance_km`，统计口径同为「按审批单取 MAX」；估算里程（routes 分段叠加）两版相同。
- 观感差异（非问题）：v2 的 `visit_note` 为「交流对象/沟通内容/存在问题」三段，v1 为「本次拜访情况」。

#### 旧表单退役

- 旧表单从钉钉工作台下架后，把 `DINGTALK_PROCESS_CODE`（v1）从 env 移除即停止同步旧单；v1 解析代码保留用于历史数据重跑。

#### 销售填报规则宣贯与 8/7 问题单待重录（2026-08-10）

**已宣贯的填报规则**（源自钉钉文档「用车表单V2 销售对应问题」）：

1. 全局禁用「虚拟客户」；
2. 住址/出差住址等统一用「住址（签到用）」，公司统一用「公司（签到用）」，**不要手写「住址」**（系统占位识别只认 `/虚拟|签到用/`，手写「住址」会被当成真实客户 → 多计拜访 + 污染客户名单）；
3. 在住址/公司等需要接待客户，务必录入客户名（手写或选择），**无客户名默认无效拜访**；
4. 终点为「住址」时，在前一个打卡点选「不前往下一地点」即可，无需打卡两次（优先级不高，仅影响地图「途N/终」展示）。

**流程**：销售在钉钉自行修改表单 → 修改成功后反馈 → 重新录入对应审批单。

**⚠️ 技术前提与风险**：

- 钉钉侧：已完成审批单要可改，需在表单设置开启「审批单修改」功能。
- 系统侧：**同步是「存在即跳过」，v2 重复行只刷新 `visit_note`/`visit_detail`**，客户名等字段不会随钉钉表单修改自动更新——必须走「重新录入」通道（待做：`reparseApproval` 脚本：重拉钉钉实例 → 更新 raw_approvals → 重解析替换 visits → 重算派生数据：拜访排除标记、routes、stops、异常、风险摘要）。

**8/7（周五）待重录问题单**（已与库中数据逐条核对）：

| 人员 | 审批单 | 问题 | 当前系统影响 |
|---|---|---|---|
| 林伟东 | 202608071057000303031 | 签1 真实客户（鑫永泰）选了「虚拟客户」 | 漏计 1 次拜访 |
| 林伟东 | 202608071332000399569 | 签1 真实客户选了「虚拟客户」 | 漏计 1 次拜访 |
| 谭国豪 | 202608071101000142591 | 公司（创维数字大厦）接待需写客户名 | 库中已写「丹弗」，现状合规，待与销售确认 |
| 罗易 | 202608070832000358617 | 文档标注「公司接待需写客户名」，但截图贴错（与谭国豪重复）；库中实际问题是签4 手写「住址」 | 手写「住址」被当真实客户：多计 1 次 + 客户名单污染 |
| 贺鹏程 | 202608071037000177135 | 签2 真实客户选了「虚拟客户」 | 漏计 1 次拜访 |
| 杨天序 | 202608070851000562349 | 签4 手写「住址」 | 多计 1 次拜访 + 客户名单混入「住址」 |
| 黄小川 | 202608070759000049995 | 签2 真实拜访（吴总）未写客户名 | 漏计 1 次拜访 |
...
**✅ 已执行：手工修正 + 重算（2026-08-10 晚）**：因钉钉侧修改有风险（杨天序事件），改为系统内一次性 SQL 手改（未走 reparseApproval、未改任何代码规则）：林伟东 ...0303031 签1 →「鑫永泰」、林伟东 ...0399569 签1 →「深圳市鸿盟精密科技有限公司」、罗易 ...0358617 签4 →「住址（签到用）」、杨天序 ...0562349 签4 →「住址（签到用）」、贺鹏程 ...0177135 签2 →「代理商定利工具」、黄小川 ...0049995 签2 →「苏州博锐鑫机械技术有限公司」（对应行 `exclude_from_visit_count`/`customer_count` 同步修正）；谭国豪单保持「丹弗」不动。8/7 异常与风险摘要已重算（refreshRiskCache）。**7 张单继续锁定**，后续同步永远跳过；这几个人有新录入由管理员手动写入。**raw_approvals 未动**（保留原始填报），注意这些单不能再用 reparseApproval（会把手工修正冲掉）。住址模糊识别补丁（06f1a0c）已随锁定机制部署（1934923）实际上线，仅对**新入库**数据生效；存量遗留：桂君 8/7 手写「住址」仍计为拜访（待决策）。
...
### Step 3.7：风险标签规则调整（进行中）

> 记录时间：2026-07-30
> 状态：部分已上线，「拜访量不足」规则待重新设计

#### 已完成（已上线）

- **全系统「拜访次数」口径排除住址/公司地址**：`visits.exclude_from_visit_count` 标记命中员工本人住址或 `company_addresses` 白名单的签到；新数据在 `processParsedVisits` 入库时自动打标，存量用 `npm run backfill:visit-exclusion` 回填（幂等，住址/公司地址变更后重跑刷新）。决策页（总拜访/周趋势/员工活跃度/部门人均）、控制台（拜访频率/人均/排行榜/拜访点数）、风险摘要 visit_count、日周月报与导出报告共约 20 处统计统一过滤；轨迹、停留点、里程不受影响。生产约 36% 签到被排除（主要为在家「出发点」与公司「回公司」打卡），拜访次数整体下降约三分之一属预期。
- **「重复签到」规则接入同一套排除**：统计前剔除本人住址 + 公司地址签到；公司地址由硬编码创维数字大厦改为读 `company_addresses` 表（`anomalyDetection.ts` 的 `filterExcludedVisits`），新增公司地址不用改代码。阈值维持同一地点 >= 3 次（>= 5 次升 high），按当前业务周（周一到当天）统计。
- **决策页雷达图**：固定对比销售部下「华南一部/华东昆山/华东宁波/华北一部」四个子部门；目标部门列表同步更新（新增新品业务部/软件业务部，移除东南一部/软件产品线）。

#### TODO：「拜访量不足」规则重新设计

当前规则（刻意保持原口径，未随拜访次数口径调整）：业务周最后一天评估，当周 visits 全部签到条数 < 阈值即触发。待重新设计，要点：

- [ ] **统计口径**：应改为只统计「有效拜访」——排除住址/公司地址（可直接复用 `exclude_from_visit_count` 标记），是否进一步限定签到类型待定。
- [ ] **签到类型过滤**：业务方曾提出排除「代理商客户签到」「其他签到」，但库里 `trip_type` 实际只有 4 个值（`拜访客户签到-开车（统计里里程）`、`拜访客户签到-公共交通-陪同拜访`、`其他特殊签到（如在办公室，拜访行业伙伴）`、`虚拟客户(合作伙伴/陌拜/展会)`），需与业务确认准确选项名后再实现。
- [ ] **分区阈值**：不同地区（子部门）的拜访量阈值可能不同，规则配置需支持按区定制（当前 `anomaly_weights` 是全局单阈值）。
- [ ] **确认阈值基线**：业务记忆是「周拜访量 < 10 次」，但 `anomaly_weights` 表里 `low_visit_count` 的 `threshold_value` 实际是 15，重新设计时一并确认。
- [ ] 「重复签到」阈值后续也可能按区调整，与拜访量规则一起设计配置结构。
- [ ] 规则定稿并上线后，历史日期的 `low_visit_count` 异常需按新口径定向重算（参考已删除的 `recomputeLowVisitCount.ts` 思路：只重算该类型异常 + 同步风险分/缓存，可从 git 历史 `bbf2cd6` 找回）。
...
- [ ] 部门归属改为「按人归属」根治：每个 user_id 唯一归属其最近一次出现的「销售部-X」子部门，该用户全部签到（含挂在「销售渠道-XX区域」下的区总记录）统一算进归属子部门（历史数据融合重归属，不删除）；没有任何销售部记录的人不进系统统计（exclude_from_stats 用户除外）。落地后废除 `department_aliases` 中 销售渠道-X区域 三条别名映射与归一化机制，组织树/排行榜/区域概览/报告分组/级联选择器/对账白名单收口到同一个「user_id → 归属部门」解析函数。背景：2026-08-31 贺鹏程/季昕亚因白名单误标无法登录（7/28 对账上线 ~ 8/20 归一化上线之间的窗口期打标，只置位不恢复导致残留）；当年度别名映射已覆盖全部现实渠道签到（华南/江苏/浙江 → 华南一部/华东昆山/华东宁波），无存量统计错误，故不紧急。
