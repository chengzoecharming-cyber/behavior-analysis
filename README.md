...
报告的「客户数统计 / 客户拜访列表」会自动排除两类签到，无需改代码，只需维护数据：

- **员工住址**：命中**任何一位**员工的住址（文本匹配或 500 米坐标半径）都不算客户，例如在家签到、出差留宿同事家小区。
- **公司地址**：命中 `company_addresses` 白名单（如创维数字大厦）的签到，对全体员工都不算客户。

**新增 / 变更员工住址**（新人入职、搬家）：

1. 更新线下收集的《国内业务人员常住地址》Excel（姓名 + 地址两列即可）。
2. 服务器上执行（镜像内已内置脚本）：

   ```bash
   docker exec sales-map-backend npm run import:addresses
   ```

   脚本是幂等 UPDATE，可反复执行；导入时会自动把住址解析成坐标存入 `users.home_lat/home_lng`（高德解析不出时会自动逐级截掉「幢/栋/单元/室」重试）。
3. 如果脚本末尾打印了「地理编码失败」名单，把原始住址和正确坐标人工写入 `address_fallback_coordinates` 表后重跑导入即可。
4. 已导入过但缺坐标的存量数据，可单独回填：`docker exec sales-map-backend npm run backfill:home-coords`。

> 注意：个别住址高德会「解析成功但位置跑偏」（兜底表管不到这种情况），表现为该员工住址仍出现在报告里。修正方式：查出正确经纬度后直接 `UPDATE users SET home_lat=..., home_lng=... WHERE user_id='...'`。

**新增 / 变更公司地址**（搬职场、新增办公点）：

```bash
docker exec sales-map-backend npm run upsert:company-address -- "创维数字大厦" "广东省深圳市宝安区石岩街道创维数字大厦"
```

重复执行按地址覆盖更新；坐标自动解析。

**生效时机**：住址/公司地址变更后，下一次定时报告（日报 9:00 / 周报周日 18:00 / 月报每月 1 日）自动生效。如需立即刷新历史报告，调 `/export/generate-reports` 手动重生成（同周期同名文档原地覆写，链接不变）。
