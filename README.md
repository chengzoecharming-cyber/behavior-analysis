...
**规则：业务日期统一按员工实际签到时间（北京时间）计算。**

无论数据来自 Excel 还是钉钉，都会把 visit 的原始时间（`visit.time`）解析为北京时间后取日期部分，作为该条记录的业务日期。

为什么不用钉钉审批单创建时间？

- 钉钉审批单创建时间可能早于实际签到（提前提交），也可能晚于实际签到（补卡）。
- 外勤行为分析关心的是「员工哪天真实发生了拜访」，而不是「审批单哪天被提交」。
- 如果按审批单创建时间，会出现「7 月 21 日出去拜访，但数据被算到 7 月 20 日」的情况，导致控制台和决策页失真。

因此：
- `business_date` = 实际签到日期（北京时间）
