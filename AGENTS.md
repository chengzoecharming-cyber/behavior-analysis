...
3. **同步健康告警**：每次钉钉同步后（无论成败，finally 中）调用 `checkAndSendAlerts()` 检查数据完整性，发现异常通过 `DINGTALK_EXPORT_ROBOT_WEBHOOK` 发送机器人告警——**多条告警合并为一条汇总消息**（自定义机器人限流 20 条/分钟，逐条发会在积压时打爆通道形成告警风暴，8/16 事故：107 条积压全是 parse_failures>0 的常规噪音）；发送失败不标记 `alert_sent`，下轮重发。每天早上 9:00 发送昨日同步健康摘要：昨日零同步日志按异常告警（调度器可能整天未运行），正常日也发简短心跳（区分「正常」与「告警通道故障」）。手动同步接口同样触发告警检查。注意：`parse_failures > 0` 也会触发告警，而「审批单解析出 0 条 visits」（如无打卡的行程）每次同步都会重复计数，属常规噪音，看汇总即可。
